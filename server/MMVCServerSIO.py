...
def setupArgParser():
    ...
    parser.add_argument("--noNativeClient", action="store_true", help="Run the web server in this process without launching the native client wrapper.")
    parser.add_argument("--deferModelLoad", action="store_true", help="Start the web server before loading the last selected model.")
...
if args.deferModelLoad:
    os.environ["VCB_DEFER_MODEL_LOAD"] = "1"
...
def createSocketIOApp():
    voiceChangerManager = VoiceChangerManager.get_instance(voiceChangerParams)
    app_fastapi = MMVC_Rest.get_instance(voiceChangerManager, voiceChangerParams, args.allowed_origins, PORT)
    return MMVC_SocketIOApp.get_instance(app_fastapi, voiceChangerManager, args.allowed_origins, PORT)
...
def localServerDirect(logLevel: str = "critical", key_path: str | None = None, cert_path: str | None = None):
    try:
        uvicorn.run(
            createSocketIOApp(),
            host=HOST,
...
if __name__ == "MMVCServerSIO":
    ...
    app_socketio = createSocketIOApp()
...
if __name__ == "__main__":
    ...
    # サーバ起動
    if args.noNativeClient:
        localServerDirect(args.logLevel, key_path, cert_path)
