...
class VoiceChangerManager(ServerDeviceCallbacks):
    ...
    def __init__(self, params: VoiceChangerParams):
        ...
        self.modelLoadState: str = "idle"
        self.modelLoadError: str = ""
        ...
        if os.path.exists(STORED_SETTING_FILE):
            self.stored_setting = json.load(open(STORED_SETTING_FILE, "r", encoding="utf-8-sig"))
        if "modelSlotIndex" in self.stored_setting:
            if os.getenv("VCB_DEFER_MODEL_LOAD") == "1":
                self.settings.modelSlotIndex = self._normalize_model_slot_index(self.stored_setting["modelSlotIndex"])
                self.modelLoadState = "deferred"
            else:
    ...
    def _normalize_model_slot_index(self, val: str | int | float | bool):
        try:
            newVal = int(val)
            return newVal % 1000
        except:
            return re.sub("^\d+", "", str(val))  # 先頭の数字を取り除く。
    ...
    def get_info(self):
        ...
        data["modelLoaded"] = self.voiceChanger is not None
        data["modelLoadState"] = self.modelLoadState
        data["modelLoadError"] = self.modelLoadError
    ...
    def update_settings(self, key: str, val: str | int | float | bool):
        ...
        if key in self.settings.boolData:
            ...
            if key == "modelSlotIndex":
                newVal = self._normalize_model_slot_index(val)
                ...
                try:
                    ...
                    self.modelLoadState = "loaded" if self.voiceChanger is not None else "empty"
                except Exception as e:
                    self.modelLoadState = "error"
                    self.modelLoadError = str(e)
                    raise
                # キャッシュ設定の反映
